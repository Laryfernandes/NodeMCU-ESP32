...
#define RESERVOIR_STATUS_TOPIC "status/reservatório" // Novo tópico para o status do reservatório
...
bool previousRelayState = false; // Estado anterior do relé
...
void sendHumidity() {
  if (deviceRunning) { // Verifica se o dispositivo está ligado
    ...
    // Verifica se houve mudança no estado do relé
    if (previousRelayState != relayBlocked) {
      if (relayBlocked) {
        mqttClient.publish(RESERVOIR_STATUS_TOPIC, "Vazio");
      } else {
        mqttClient.publish(RESERVOIR_STATUS_TOPIC, "Normal");
      }
